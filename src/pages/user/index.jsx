...
export default function Profile() {
  ...
  useEffect(() => {
    if (user.id !== profile.id) navigate("/", { replace: true });
    getCountRecipes();
  }, []);
  ...
  const handleSubmitAvatar = async () => {
    ...
    if (profile.avatar) await deleteAvatar(profile.avatar);
        ...
        <div className="mb-3 flex">
          ...
          <div className="self-end mb-3 ml-2">
            {user.email_confirmed_at ? (
              <span className="px-3 py-2 rounded-lg bg-primary">Confirmed</span>
            ) : (
